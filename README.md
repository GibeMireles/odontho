...
- **Google Analytics (GA4) activo y verificado en producción**: `clinica.google_analytics_id` en `config.json` tiene el Measurement ID real (`G-XV2S537RW2`) y `initGoogleAnalytics()` en `main.js` inyecta `gtag.js` al cargar — nada hardcodeado en el HTML, mismo patrón data-driven del resto del sitio. Flujo de datos en GA4: nombre "Odonthó", URL `https://gibemireles.github.io/odontho/`, Stream ID `15742474179`. Verificado en local (carga el script y registra los eventos `js`/`config` en `dataLayer`) y confirmado que `https://gibemireles.github.io/odontho/config.json` ya sirve el ID real en producción (GitHub Pages tardó ~15-30s en propagar el deploy tras el push).
...
```jsonc
{
  "clinica": {
    ...
    "google_analytics_id": "G-XXXXXXXXXX", // ← Measurement ID de GA4; null = analytics desactivado
    "horario": { ... }
  },
  "doctores": [
    {
