...
/* Intenta renderizar <img> con fallback a ícono si la imagen no existe */
function renderFoto(src, nombre) {
  const fallback = icono('usuario', 'ic ic-avatar').replace(/"/g, '&quot;');
  if (!src) return icono('usuario', 'ic ic-avatar');
  return `<img src="${src}" alt="${nombre}" onerror="this.parentElement.innerHTML='${fallback}'">`;
}
...
function renderFooter() {
  ...
  document.getElementById('pie-contacto-links').innerHTML = `
    <a href="https://maps.google.com" target="_blank" rel="noopener">${icono('ubicacion')} ${c.direccion}</a>
